...
static inline uint16_t crc16_low(const uint16_t* lookup, const void* data, size_t length, uint16_t last_chunk_crc) {
    ...
    while (length--) {
        uint8_t temp = *bdata++ ^ crc_word;
        crc_word >>= 8;
        crc_word ^= lookup[temp];
    }
    return crc_word;
}
...
static inline uint16_t crc16_high(const uint16_t* lookup, const void* data, size_t length, uint16_t last_chunk_crc) {
    ...
    const uint8_t* bdata = (const uint8_t*)data;
    while (length--) {
        uint8_t temp = (crc_word >> 8) ^ *bdata++;
        crc_word = ((crc_word << 8) & 0xff00) ^ lookup[temp];
    }
    return crc_word;
}

uint16_t rlib::crc16_modbus(const void* data, size_t length, uint16_t last_chunk_crc) {
    ...
    return crc16_low(crc_lookup, data, length, last_chunk_crc);
}

uint16_t rlib::crc16_ccitt_ffff(const void* data, size_t length, uint16_t last_chunk_crc) {
    static const uint16_t crc_lookup[] =  //
    ...
    return crc16_high(crc_lookup, data, length, last_chunk_crc);
}
